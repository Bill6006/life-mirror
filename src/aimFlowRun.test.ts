...
describe('naming an older study commitment', () => {
  ...
  it('adopts the one orphan subject, with its ladder, and otherwise waits', async () => {
    ...
    await db.skills.add({ name: 'Ten words', subject: 'Veltish', ladder: 'language', order: 1, createdAt: '', archivedAt: null })
    await adoptOrphanSubjects()
    expect((await studyAims())[0]).toMatchObject({ name: 'Veltish', ladder: 'language' })
    await db.skills.add({ name: 'Scales', subject: 'Ocarina', order: 2, createdAt: '', archivedAt: null })
    await db.aims.add({ kind: 'certification', stepMoveId: null, createdAt: '', archivedAt: null })
    await adoptOrphanSubjects()
    expect((await studyAims()).map((a) => a.name ?? null)).toEqual(['Veltish', 'Ocarina'])
  })

  it('waits when two subjects are orphaned, and takes a name typed on the card', async () => {
    await db.aims.add({ kind: 'certification', stepMoveId: null, createdAt: '', archivedAt: null })
    await db.skills.bulkAdd([
      { name: 'a', subject: 'Veltish', order: 1, createdAt: '', archivedAt: null },
      { name: 'b', subject: 'Ocarina', order: 2, createdAt: '', archivedAt: null },
    ])
    await adoptOrphanSubjects()
    expect((await studyAims())[0].name).toBeUndefined()
    await nameAim(1, 'Veltish')
    expect((await studyAims())[0].name).toBe('Veltish')
  })
  ...
  it('files a skill added on the card under the commitment’s name and ladder', async () => {
    ...
    await addSkill('Scale of C', 'Ocarina')
    expect((await db.skills.toArray())[0]).toMatchObject({ subject: 'Ocarina', ladder: 'craft' })
  })

  it('names an older commitment and chooses its proofs in the same tap; its skills follow', async () => {
    await db.aims.add({ kind: 'certification', stepMoveId: null, createdAt: '', archivedAt: null })
    await db.skills.add({ name: 'Ten words', subject: 'Veltish', order: 1, createdAt: '', archivedAt: null })
    await nameAim(1, 'Veltish', 'language')
    expect((await studyAims())[0]).toMatchObject({ name: 'Veltish', ladder: 'language' })
...
describe('the ladder a subject climbs', () => {
  ...
  it('aligns every skill to its commitment’s ladder at open; a commitment without one takes the ladder its skills share, else waits', async () => {
    ...
    await db.skills.add({ name: 'Ten words', subject: 'Veltish', order: 1, createdAt: '', archivedAt: null })
    await db.aims.add({ kind: 'certification', stepMoveId: null, name: 'Ocarina', createdAt: '', archivedAt: null })
    await db.skills.add({ name: 'Scale of C', subject: 'Ocarina', ladder: 'craft', order: 2, createdAt: '', archivedAt: null })
    ...
    expect(aims.find((a) => a.name === 'Ocarina')?.ladder).toBe('craft')
  ...
  it('keeps an older commitment’s marks when its six proofs are brought into line, and its step is the skill, never a rung (Workstream 6)', async () => {
    await addAim('certification', null, 'Clockwork')
    ...
    expect(stepFor(aim, await db.skills.toArray(), await db.rungMarks.toArray(), [aim])).toMatchObject({ id: 'skill:1', name: 'Gear trains', kind: 'skill' })
  })
})
...
describe('one tap says when', () => {
  ...
  it('records the cue for today, a later tap replaces it, and Start keeps the plan', async () => {
    await addLearning('Veltish', 'A phrasebook', 'Understand spoken Veltish')
    const [aim] = await studyAims()
    ...
    expect(offer).toMatchObject({ moveId: 'skill:1', label: 'Understand spoken Veltish' })
