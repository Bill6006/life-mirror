...
describe('depth, frequency, quiet hours and Low-demand mode', () => {
  ...
  it('fills fields an older stored record lacks', () => {
    ...
    expect(s.week.churchDay).toBe(6)
    expect(s.week.livesWithMe).toBe(true)
    expect(s.week.studyNights[3]).toBe(false)
    expect(s.week.pickupTime).toBeNull()
    expect(s.direction).toBeNull()
    expect(s.directionAskedAt).toBeNull()
