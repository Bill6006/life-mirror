...
describe('a commitment and its protected step', () => {
  it('takes a learning commitment’s step from its current skill, before adoption from the skill the retired ladder named, and with none offers nothing to start (Workstream 6)', () => {
    ...
    const skills = [skill(1, 'Gear trains')]
    // Before adoption: the skill the ladder's step named, as a session of that skill, never a rung, whatever its marks.
    expect(stepFor(aim('certification'), skills, [])).toMatchObject({ id: 'skill:1', name: 'Gear trains', kind: 'skill' })
    ...
    const current: Skill = { ...skill(2, 'Understand spoken Veltish'), aimId: 1, method: 'A phrasebook', minutes: 30 }
    expect(stepFor(named, [...skills, current], [])).toMatchObject({ id: 'skill:2', name: 'Understand spoken Veltish', method: 'A phrasebook', minutes: 30, kind: 'skill' })
...
describe('the last fact on a row', () => {
  it('dates the last done step by the day it was started, and words the gap as a fact, never a streak', () => {
    ...
    const study: Aim = { id: 1, kind: 'certification', stepMoveId: null, name: 'Veltish', createdAt: '', archivedAt: null, currentSkillId: 1 }
  ...
  it('counts a skill’s practice since it became current: sessions and the different days they began, a second the same day adding a session and never a day (Workstream 6)', () => {
    const sk: Skill = { ...skill(1, 'Understand spoken Veltish'), startedAt: '2026-09-05T15:00:00.000Z' }
...
describe('several study subjects', () => {
  ...
  it('gives each study commitment the skills under its name, the unnamed ones to the first', () => {
    const a = study(1, 'Clockwork')
    const b = study(2, 'Veltish')
    const skills = [sk(1, 'Gear trains', 1), sk(2, 'Ten words', 2, 'Veltish'), sk(3, 'Escapements', 3, 'Clockwork')]
    expect(skillsOf(a, skills, [a, b]).map((s) => s.name)).toEqual(['Gear trains', 'Escapements'])
    ...
    expect(stepFor(a, skills, [], [a, b]).name).toBe('Gear trains')
  })
