import { isParked, isProposed, moves, type Move } from './catalogue'
...
export function studyVersions(offeredToday: readonly string[]): Move[] {
  return moves.filter((m) => !isProposed(m) && !isParked(m) && m.family === 'study' && m.when.includes('evening') && !offeredToday.includes(m.id))
}
...
export function smallerThan(move: Move): Move | null {
  ...
  for (const m of moves) {
    if (isProposed(m) || isParked(m) || m.family !== 'study' || m.id === move.id || m.minutes >= move.minutes) continue
